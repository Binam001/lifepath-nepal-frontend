...
export default function ComparisonCTA() {
  ...
  useEffect(() => {
    ...
    setStatus({
      ...
      hasOcean: !!ocean,
      hasOmegaverse: !!omegaverse && !!omegaverseTypes[omegaverse.toUpperCase()],
    });
  }, []);
  ...
  const completedCount = [hasMbti, hasOcean, hasOmegaverse].filter(Boolean).length;
      ...
      {allCompleted ? (
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-extrabold text-zinc-950 mb-1 flex items-center gap-2">
                Triple Alignment Dashboard Unlocked
                <span className="text-[10px] font-black tracking-widest text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
                  Ready
                </span>
              </h3>
              <p className="text-sm text-zinc-500 max-w-xl leading-relaxed">
                Awesome! You have completed all 3 personality assessments. View your
                cross-framework alignment dashboard to identify overlapping Golden Careers.
              </p>
            </div>
          </div>
          <Link href="/personality-test/comparison" className="shrink-0 w-full md:w-auto">
            <button className="w-full md:w-auto py-3.5 px-6 font-bold rounded-full bg-zinc-950 hover:bg-zinc-800 text-white text-sm transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer shadow-xs hover:scale-[1.02] active:scale-[0.98]">
        ...
        <div className="relative z-10">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-500 max-w-xl leading-relaxed">
                You have completed {completedCount} / 3 assessments. Complete the remaining tests below
                to unlock your synthesized nature profile and cross-test career overlaps!
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
            {untested.map((test) => (
              <div
                key={test.key}
                className="bg-white rounded-2xl p-4 border border-zinc-100 hover:border-zinc-200 transition-all flex flex-col justify-between"
              >
                <div>
                  <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400">
                    Untested Assessment
                  </span>
                  <h4 className="font-extrabold text-zinc-950 text-sm mt-0.5">
                    {test.name}
                  </h4>
                ...
                <Link href={test.url} className="block mt-auto">
                  <button
                    className={`w-full py-2 px-4 text-xs font-bold text-white rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs hover:scale-[1.02] ${test.btnColor}`}
                  >
                    Start Test
                    <ArrowRight size={14} />
                  </button>
            ...
            {completedCount >= 2 && (
              <div className="bg-linear-to-b from-zinc-50/50 to-zinc-50/20 border border-dashed border-zinc-200 rounded-2xl p-4 flex flex-col justify-center items-center text-center">
                ...
                <Link href="/personality-test/comparison" className="block w-full">
                  <button className="w-full py-2 px-3 bg-zinc-900 hover:bg-zinc-800 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1 cursor-pointer transition-all">
